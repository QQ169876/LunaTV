...
> 📌 **服务端去广告**：本文档描述的是**浏览器端**用法（只在网页播放页生效）。
> 若需要 TV 端（Selene-TV）、手机端（Selene）、第三方播放器也拿到干净的 m3u8，
> 请同时阅读 [服务端去广告（全端生效）](SERVER_AD_FILTER.md)。
> 服务端沿用**同一份**自定义代码与同一个函数签名 `filterAdsFromM3U8(type, m3u8Content)`，
> 后台配置一次即可，两端共用。
