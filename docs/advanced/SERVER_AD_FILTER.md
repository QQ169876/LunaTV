...
- 开启后：详情接口吐出的地址指向本站，代理会取回上游 m3u8 → 过滤 → 改写。此时
  - **网页端**：输出**相对地址** `/api/proxy/m3u8?...`（天然同源，多个域名 / IP 访问都不会跨域，Cookie 也带得上），
    分片一律走 `/api/proxy/segment`。因为多数上游分片没有 CORS 头，浏览器直连会被拦，
    表现为"网页端播不动"，所以网页端固定走同源代理；清单已由服务端过滤，播放器里不会再过滤一遍。
  ...
  - `ProxyPlaybackAllowCORS=true`：只代理几 KB 的播放列表，分片直连上游，省带宽，但要求上游分片返回 CORS 头；
    该开关**只对非浏览器客户端生效**，网页端始终走同源分片代理。
